...
function calculateQuotation(servicesText) {
  ...
  for (const service of servicesArr) {
    if (service.includes("chisperos")) {
      ...
      // En este flujo se maneja por otro estado, por lo que aquí solo se muestra un mensaje
...
async function handleUserMessage(from, userMessage, messageLower) {
  ...
  if (context.estado === "EsperandoTipoEvento") {
    ...
    await sendWhatsAppMessage(from, `¡Perfecto! Has seleccionado: ${context.tipoEvento} 👍. ¿Deseas el paquete sugerido o prefieres armar tu propio paquete? Escribe "paquete" para el paquete sugerido o "armar" para personalizar.`);
  ...
  if (context.estado === "OpcionesSeleccionadas") {
    if (messageLower.includes("armar")) {
      ...
      await sendWhatsAppMessage(from, "😕 No entendí tu respuesta. Por favor escribe 'paquete' o 'armar'.");
      return true;
    }
  }
