...
const formatFechaEnEspanol = (ddmmyyyy)=>{ const [d,m,y]=ddmmyyyy.split('/'); const meses=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"]; return `${d} de ${meses[+m-1]} ${y}`; };
const parseFecha = (s)=>{ const t=(s||"").trim(); if(/^\d{2}\/\d{2}\/\d{4}$/.test(t)) return t; const m=t.match(/^(\d{1,2})\s*(?:de\s+)?([a-záéíóú]+)\s*(?:de\s+)?(\d{4})$/i); if(!m) return null; const day=m[1].padStart(2,'0'); const map={enero:"01",febrero:"02",marzo:"03",abril:"04",mayo:"05",junio:"06",julio:"07",agosto:"08",septiembre:"09",setiembre:"09",octubre:"10",noviembre:"11",diciembre:"12"}; const month=map[normalizeText(m[2])]; if(!month) return null; return `${day}/${month}/${m[3]}`; };
const isValidDate = (ddmmyyyy)=>{ if(!/^\d{2}\/\d{2}\/\d{4}$/.test(ddmmyyyy)) return false; const [d,m,y]=ddmmyyyy.split('/').map(Number); const dt=new Date(y,m-1,d); return dt.getFullYear()===y && dt.getMonth()+1===m && dt.getDate()===d; };
const isValidDateExtended = (s)=>{ const f=parseFecha(s); return !!(f && isValidDate(f)); };
const isValidFutureDate = (s)=>{ const f=parseFecha(s); if(!f||!isValidDate(f)) return false; const [d,m,y]=f.split('/').map(Number); const inDate=new Date(y,m-1,d); const today=new Date(); today.setHours(0,0,0,0); return inDate>=today; };
const isWithinTwoYears = (s)=>{ const f=parseFecha(s); if(!f||!isValidDate(f)) return false; const [d,m,y]=f.split('/').map(Number); const inDate=new Date(y,m-1,d); const today=new Date(); today.setHours(0,0,0,0); const max=new Date(today); max.setFullYear(max.getFullYear()+2); return inDate<=max; };
const toISO = (ddmmyyyy)=>{ const [d,m,y]=ddmmyyyy.split('/'); return `${y}-${m}-${d}`; };
const shouldSkipDuplicateSend = (to, key)=>{ const k=`${to}::${key}`.slice(0,512); if(antiDuplicateCache.get(k)) return true; antiDuplicateCache.set(k,true); return false; };

/* ===== CRM helpers ===== */
async function getContext(from){ try{ const { data } = await axios.get(`${CRM_BASE_URL}/leads/context`, { params:{ telefono: from }, timeout: 5000 }); return data?.context || null; }catch(e){ console.error("getContext:", e.message); return null; } }
async function saveContext(from, context){ try{ await axios.post(`${CRM_BASE_URL}/leads/context`, { telefono: from, context: { ...context, lastActivity: new Date().toISOString() } }, { timeout: 5000 }); }catch(e){ console.error("saveContext:", e.message); } }
async function reportMessageToCRM(to, message, tipo="enviado"){ try{ await axios.post(`${CRM_BASE_URL}/recibir_mensaje`, { plataforma:"WhatsApp", remitente: to, mensaje: message, tipo }, { timeout: 7000 }); }catch(e){ console.error("CRM report:", e.response?.data || e.message); } }
async function reportEventToCRM(to, eventName, meta={}){ const payload = `EVENT:${eventName}${Object.keys(meta).length?` ${JSON.stringify(meta)}`:''}`; await reportMessageToCRM(to, payload, "enviado"); }
async function checkAvailability(dateYYYYMMDD){ try{ const { data } = await axios.get(`${CRM_BASE_URL}/calendario/check`, { params:{ fecha: dateYYYYMMDD }, timeout: 7000 }); return !!data?.available; }catch(e){ console.error("disponibilidad:", e.message); return false; } }
...
async function sendWhatsAppMessage(to, text){
  try{
    ...
    const { data } = await axios.post(WABA_URL, { messaging_product:'whatsapp', to, type:'text', text:{ body: text } }, { headers: WABA_HEADERS, timeout: 15000 });
...
async function sendImageMessage(to, imageUrl, caption){
  try{
    ...
    const { data } = await axios.post(WABA_URL, { messaging_product:'whatsapp', to, type:'image', image:{ link, ...(caption?{caption}:{}) } }, { headers: WABA_HEADERS, timeout: 20000 });
...
async function sendWhatsAppVideo(to, videoUrl, caption){
  try{
    ...
    const { data } = await axios.post(WABA_URL, { messaging_product:'whatsapp', to, type:'video', video:{ link, ...(caption?{caption}:{}) } }, { headers: WABA_HEADERS, timeout: 30000 });
...
function detectIntentRegex(text){
  ...
  if(/^paquete wedding$|^wedding\b|^paquete boda$/.test(t)) return INTENTS.BUTTON_CONF_PAWED;
...
async function handleLugar(from, userText, context){
  ...
  await delay(500);
  await sendMessageWithTypingWithState(from, "Después de tu depósito:\n1) Te pido tus datos\n2) Llenamos el contrato\n3) Te envío foto del contrato firmado", 200, "Finalizando");
  await delay(400);
...
async function flujoXVDespuesDeFechaOK(from, context, pretty){
  ...
  await delay(600);
  await sendMessageWithTypingWithState(from, "🎁 *PROMO:* Al contratar este paquete te llevas *Audio Guest Book* sin costo.", 200, context.estado);
  await delay(600);
...
async function handleArmarMiPaquete(from, context){
  ...
  await sendMessageWithTypingWithState(
    from,
    "🧩 *ARMAR MI PAQUETE*\nAbre este enlace corto. Si ya tengo tu fecha, te paso directo al Paso 3:",
    200,
    "EnviandoACotizador"
  );
...
async function handleIntent(from, context, intent){
  switch(intent){
    ...
    case INTENTS.PRICE:
      ...
      await sendWhatsAppMessage(from, "Los *precios* los ves en tiempo real en *ARMAR MI PAQUETE*.");
      await sendMessageWithTypingWithState(from, `Ingresa con el botón 🧩 *ARMAR MI PAQUETE*`, 200, (await getContext(from))?.estado || context.estado);
...
app.post('/webhook', async (req,res)=>{
  try{
    ...
    try{ await axios.post(`${CRM_BASE_URL}/recibir_mensaje`, { plataforma:"WhatsApp", remitente: from, mensaje: userMessage }); }
...
app.post('/upload_imagen', upload.single('imagen'), async (req,res)=>{
  try{ if(!req.file) return res.status(400).json({ error:"No se recibió archivo en 'imagen'" });
    const up = await s3.upload({ Bucket: process.env.S3_BUCKET_NAME, Key: `${uuidv4()}_${req.file.originalname}`, Body: req.file.buffer, ContentType: req.file.mimetype || 'application/octet-stream' }).promise();
    return res.json({ url: up.Location });
  }catch(e){ console.error("upload_imagen:", e.message); return res.status(500).json({ error:e.message }); }
});
app.post('/enviar_mensaje', async (req,res)=>{ try{ const { telefono, mensaje } = req.body; if(!telefono||!mensaje) return res.status(400).json({ error:'Faltan datos' }); await sendWhatsAppMessage(telefono, mensaje); res.json({ ok:true }); }catch(e){ console.error("enviar_mensaje:", e.message); res.status(500).json({ error:'Error al enviar a WhatsApp' }); } });
app.post('/enviar_imagen', async (req,res)=>{ try{ const { telefono, imageUrl, caption } = req.body; if(!telefono||!imageUrl) return res.status(400).json({ error:'Faltan datos (telefono, imageUrl)' }); await sendImageMessage(telefono, imageUrl, caption); res.json({ ok:true }); }catch(e){ console.error("enviar_imagen:", e.message); res.status(500).json({ error:'Error al enviar imagen' }); } });
app.post('/enviar_video', async (req,res)=>{ try{ const { telefono, videoUrl, caption } = req.body; if(!telefono||!videoUrl) return res.status(400).json({ error:'Faltan datos (telefono, videoUrl)' }); await sendWhatsAppVideo(telefono, videoUrl, caption); res.json({ ok:true }); }catch(e){ console.error("enviar_video:", e.message); res.status(500).json({ error:'Error al enviar video' }); } });
